...
def getImportances(classifier, features_list, savefig=None):
    ...
    if savefig != None:
        indices = indices[::-1]
        y_pos = np.arange(len(features_list[indices]))
        plt.rcdefaults()
        plt.clf()
        plt.barh(y_pos, importances[indices], align='center', alpha=0.5)
        plt.yticks(y_pos, features_list[indices])
        plt.xlim([0,np.max(importances)+0.03])
        plt.title("Feature importances")
        for i in range(len(importances[indices])):
            plt.annotate('{:.4f}'.format(importances[indices[i]]), xy=(importances[indices[i]],y_pos[i]))
        plt.tight_layout()
        print("Saving the feature importance scores as {}".format(savefig))
        plt.savefig(savefig)
